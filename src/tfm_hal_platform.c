...
/* Enable full NS access to coprocessors CP0-CP7 (GPIO/DCP/RCP/etc). */
#define CPACR_CP0_7_FULL_ACCESS 0x0000FFFFU
#define NSACR_CP0_7_ENABLE      0x000000FFU
...
{
    /* Allow NS access to CP4 (DCP) before handing off to the NS image. */
    SCB->CPACR |= CPACR_CP0_7_FULL_ACCESS;
    SCB_NS->CPACR |= CPACR_CP0_7_FULL_ACCESS;
    SCB->NSACR |= NSACR_CP0_7_ENABLE;
    __DSB();
    __ISB();
    ...
    /* Route USB controller IRQ to the Non-secure state for CDC ACM. */
    NVIC_SetTargetState(USBCTRL_IRQ_IRQn);
